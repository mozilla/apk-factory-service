...
withConfig(function(config) {
  ...
  module.exports = function(manifestUrl, manifestOverride, loader,
    ...
    loader.load(manifestFilename, function(err, string) {
      if (err) {
        log.error("OWA Cannot load manifest: " + manifestFilename);
        return cb(err);
      }
      try {
        ...
        if (false === owaValidator(manifest)) {
          return cb(new Exception('invalid manifest'));
        }
        ...
        if (appType === "hosted") {
          ...
          request({
            ...
            var zip;
            if (err) {
              log.error("OWA Error downloading package " + packagePath);
              return cb(err);
            } else {
        ...
        log.error("OWA Error downloading " + manifestUrl);
        cb(e);
      }
    });
  };
