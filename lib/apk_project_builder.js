...
function signApk(id, apkLocation, unsignedApkPath, signedApkPath, cb) {
  ...
  fs.stat(apkLocation, function(err, stat) {
    if (err) {
      self.log.error('stat failed for ' + apkLocation);
      return cb(err);
    }
    fs.readFile(apkLocation,
                ...
                function(err, data) {
                  if (err) {
                    self.log.error('read file failed for ' + apkLocation);
                    return cb(err);
                  }
                  shasum.update(data);
                  ...
                  s3.saveApk(unsignedApkPath, data, stat.size, function(err) {
                    ...
                    signer.post('/sign', {
                      ...
                      if (err) {
                        self.log.error('Problem with /sign request');
                        return cb(err);
                        ...
                        try {
                          ...
                          self.log.error('Unable to parse ' + rawBody);
                          return cb(e);
                        }
                      }
                    });
