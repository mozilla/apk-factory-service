...
_.extend(ApkProject.prototype, {
  ...
  build: function(keyDir, cb) {
    ...
    var libProj = path.relative(self.dest,
                                path.resolve(__dirname, "ext", "apk-factory-library"));
    ...
    function antCb(error, stdout, stderr) {
      var releasePath = self._sanitize(
        xmlEscape(
          self.manifest.name)) + "-release.apk";
    ...
    if (!fs.existsSync(keyFile)) {
      ...
      var genKeyCommand = this.env.render("keygen.sh", {
        ...
        commonName: x509(developer.name || "Unknown"),
        organizationUnit: x509(developer.url || "Unknown"),
        organization: x509(hostname),
        city: "Unknown",
        state: "Unknown",
        countryCode: "XX"
      });
