...
module.exports = function(manifestUrl, config, log, cb) {
  ...
  function doCacheMiss(isUpdate) {
    ...
    // Miss
    cb(null, null, function(newApkCacheFile, ourApkVersion, apkCachedCb) {      
      ...
      apkHash(manifestUrl, function(err, manifestHash, packageHash) {
        log.info('Updating apk metadata manifest=' + manifestHash + ' package=' + packageHash + ' for ' + manifestUrl);
    ...
    apkHash(manifestUrl, function(err, manifestHash, packageHash) {
      if (err) {
        ...
        if (packageHash !== meta.package_hash) {
          log.info('cached apk package does not match reality ' +
                   packageHash + ' !== ' + meta.package_hash + ' doing cache miss');
          return doCacheMiss();
        }
      }
      // Hit
