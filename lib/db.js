...
var AQUIRE_LOCK_SQL = "INSERT INTO apk_build_lock (manifest_hash, last_modified) " +
  "VALUES (?, ?)";

var RELEASE_LOCK_SQL = "DELETE FROM apk_build_lock WHERE manifest_hash = ?";

var SELECT_LOCKS_SQL = "SELECT manifest_hash, last_modified FROM apk_build_lock";
...
withConfig(function(config) {
  ...
  exports.aquireBuildLock = function(manifestHash, config, cb) {
    log.info('DB Aquiring build lock ' + manifestHash);
    var params = [ manifestHash, new Date() ];
    var conn = mysql.createConnection(config.mysql);
    try {
      conn.connect();
      conn.query(AQUIRE_LOCK_SQL, params, function(err /*, rows, fields*/ ) {
        conn.end();
  ...
  exports.releaseBuildLock = function(manifestHash, config, cb) {
    ...
    try {
      conn.connect();
      conn.query(RELEASE_LOCK_SQL, params, function(err, rows) {
        if (err) {
  ...
  exports.activeBuildLocks = function(config, cb) {
    ...
    try {
      conn.connect();
      conn.query(SELECT_LOCKS_SQL, null, function(err, rows) {
        conn.end();
        return cb(err, rows);
      });
    } catch (e) {
