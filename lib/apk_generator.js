...
function moveApkToS3(newApkCacheFile, config, packageName, cb) {
...
withConfig(function(config) {
  ...
  _.extend(ApkGenerator.prototype, {
    ...
    generate: function(manifest, zip, loadDir, cb) {
      ...
      console.log('AOK appBuildDir=', appBuildDir);
      ...
      function doGen(err, manifest, miniManifest, zipFileLocation,
        ...
        function onCreate(androidManifestProperties) {
          ...
          projectBuilder.build(self.keysDir, function(err, apkLoc) {
            ...
            if (noCache) {
              return cb(null, apkLoc);
            } else {
              apkLoc = moveApkToS3(apkLoc, config, packageName, function(err, apkLoc) {
                if (err) {
                  console.log(err);
                  console.log(err.stack);
                  return cb(err);
