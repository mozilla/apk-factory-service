...
function moveApk(newApkCacheFile, config, packageName, cb) {
  ...
  // newApkCacheFile= /tmp/android-projects/org.mozilla.people.openwebapps.xfdesre/bin/FbImport-release.apk
  // apkCachedFile= /tmp/apk-cache/apks/org.mozilla.people.openwebapps.xfdesre/application.apk
  // AOK
  ...
  var source = fs.createReadStream(newApkCacheFile);
  var dest = fs.createWriteStream(apkCachedFile);
...
withConfig(function(config) {
  _.extend(ApkGenerator.prototype, {
    generate: function(manifest, projectBuilder, zipFileLocation,
      projectBuilderDest, loader, cb) {
      var self = this;
