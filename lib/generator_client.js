...
module.exports = function(config, manifest, zip, loadDir, log, cb) {
  ...
  request({
    ...
    if (err || 200 !== res.statusCode) {
      ...
      cb(err ||
        new Error('Generator response status code was ' + res.statusCode), res.statusCode);
    } else {
      var data = {};
      try {
        ...
        data.status = 'error';
        logError(log, 'Problem parsing generator response ' + body, e);
      }
      if ('okay' === data.status) {
        var locationType;
        ...
        if (locationType === data.locationType) {
          ...
          cb(new Error('Deployment error, expected ' + locationType + ', but got ' +
            data.apkFileLocation), res.statusCode);
        }
      } else {
        cb(new Error('Error in generator - ' + body), res.statusCode);
      }
    }
  });
};
