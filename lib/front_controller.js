...
function cacheMissGenerateAPK(manifestUrl, config, cacheApkFn, cb, finishedCb) {
  ...
  owaDownloader(manifestUrl, loader, projectBuilder.dest, appBuildDir, function(err, manifest, miniManifest, zipFileLocation, projectBuilderDest, loader) {
    if(err) {
      finishedCb();
      return cb(err);
    }
    if (projectBuilderDest) {
      console.log('proj dest changing',    projectBuilder.dest, projectBuilderDest);
    ...
    generator.generate(manifestUrl, appType, projectBuilder, manifest, miniManifest, zipFileLocation, loader, function(err, apkFilepath) {
      if (!err) {
