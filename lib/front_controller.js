  ...
  buildQueue(manifestUrl, function(finishedCb) {
    apkCache(manifestUrl, config, function(err, apkInfo, cacheApkFn) {
      if (err) {
        ...
        loadApk(apkInfo.s3Key, cb);
        finishedCb();
      } else {
        cacheMissGenerateAPK(manifestUrl, appType, config, cacheApkFn, cb, finishedCb);
      }
    });
  });
};
...
function cacheMissGenerateAPK(manifestUrl, appType, config, cacheApkFn, cb, finishedCb) {
  var loaderDirname;
  ...
  function owaCb(err, manifest, zip) {
    if (err) {
      finishedCb();
      return cb(err);
    }
    ...
    manifest.version = ourApkVersion;
    ...
    function genCb(err, apkFilepath) {
      if (err) {
        ...
        cacheApkFn(apkFilepath, ourApkVersion, function(err) {
          // Tell buildQueue we're finished
          finishedCb();
          if (err) {
            cb(err);
          } else {
            loadApk(packageName, cb);
          }
        });
      }
    }
