...
function cacheMissGenerateAPK(manifestUrl, config, cacheApkFn, cb, finishedCb) {
  ...
  var buildDest = path.resolve(process.cwd(), appBuildDir);

  owaDownloader(manifestUrl, loader, buildDest, appBuildDir, owaCb);

  function owaCb(err, manifest, zipPath, projBuildDest, loader) {
    if (err) {
      finishedCb();
      return cb(err);
    }
    ...
    if (typeof zipPath === 'string' &&
      false === fs.existsSync(zipPath)) {
      zipPath = null;
    }

    generator.generate(manifestParams, zipPath, projBuildDest, loaderDirname, genCb);
